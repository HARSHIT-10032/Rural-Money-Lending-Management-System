...
const SettleLoan = () => {
  ...
  // Fetch loan from context or API
  useEffect(() => {
    const fetchLoan = async () => {
      try {
        let found = (loans || []).find((l) => l._id === id);
        if (!found) {
  ...
  if (!loan) return <p className="text-center">Loan not found or already cleared.</p>;
  ...
  const confirmPayment = async ({ amount, type, method, date }) => {
    try {
      ...
      if (data?.loan) updateLoan(data.loan);
  ...
  const handleConfirmTotal = () => {
    const total = (loan.amount || 0) + (loan.totalInterest || 0);
    confirmPayment({ amount: total, type: "Settlement", method: paymentMethod, date: paymentDate });
  };

  const handleInterestPayment = () => {
    const amt = Number(paymentAmount);
    if (!amt || amt <= 0) return alert("Enter a valid amount");
    if (amt > (loan.totalInterest || 0)) return alert("Cannot exceed outstanding interest");
    confirmPayment({ amount: amt, type: "Interest", method: paymentMethod, date: paymentDate });
  };
      ...
      <div className="card-box">
        ...
        <table className="detail-table">
          <tbody>
            <tr>
              <th>Name</th>
              <td>{loan.user ? `${loan.user.name} ${loan.user.middleName || ""} ${loan.user.surname}` : "-"}</td>
            </tr>
            <tr>
              <th>Aadhaar</th>
        ...
        <table className="detail-table">
          <tbody>
            ...
            <tr>
              ...
              <td>₹{Number((loan.amount || 0) + (loan.totalInterest || 0)).toLocaleString()}</td>
            </tr>
          </tbody>
        </table>
      </div>
        ...
        <div className="settle-card">
          ...
          {settleOption === "compound" && (
            <div className="info-box">
              <p>Principal will be updated for next period interest.</p>
            </div>
          )}
        </div>
      </div>
